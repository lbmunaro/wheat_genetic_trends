...
Create a list with data frames for each year

```{r}
# Create a list with a data.frame for each year
list_of_data <- split(blues.na, f=blues.na$year)

# Create an empty list to store results for each year
genetic_corr_list <- list()
...
for (i in 1:length(list_of_data)) {
    ...
    # Extract variance components as per your original code
    var <- as.vector(unique(current_data$var))
    mat_varcomp <- 
      as.data.frame(summary(mt_mod)$varcomp) |>
      dplyr::select(component) |>
      rownames_to_column('varcomp') |>
      filter(str_detect(varcomp, paste(var, collapse = '|'))) |>
      mutate(varcomp = str_replace_all(varcomp, 'var:gen!var_', '')) |>
      separate(varcomp, c('var1', 'var2'), sep = ':') |>
      spread(var2, component) |>
      column_to_rownames('var1') |>
      as.matrix() |>
      t()
    
    # Function to calculate genetic correlations as per your original code
    calc_gen_corr <- function(mat) {
      ...
      for (i in 1:(n - 1)) {
        for (j in (i + 1):n) {
          genetic_corr <- mat[i, j] / (sqrt(mat[i, i]) * sqrt(mat[j, j]))
          genetic_corr_mat[i, j] <- genetic_corr
          genetic_corr_mat[j, i] <- genetic_corr
        }
      }
      return(genetic_corr_mat)
    }
    
    # Calculate genetic correlations
    genetic_corr <- calc_gen_corr(mat_varcomp)
    
    # Store genetic correlations for the current year in the list
    genetic_corr_list[[i]] <- genetic_corr
}
```
